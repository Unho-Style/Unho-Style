...
class chatManager {
    ...
    static createChat(tradeId, ownerId, buyerId) {
        ...
        let chat_id;
        try{
            result = chatDB.prepare('INSERT INTO chat(trade_id, owner_id, buyer_id, last_message) VALUES (?, ?, ?, 0);')
    ...
    static sendMessage(chatId, senderId, content) { 
        ...
        try{
            ...
            let data = this.getChatByChatId(chatId)
            if(data.success) {
                let sender_username = userManager.getUserInfoById(senderId);
                if(sender_username.success) userManager.sendPushNoti(data.result.owner_id, sender_username.result.username, content);
            }
        }catch{}finally{chatDB.close()}
    ...
    static getChatsByUserId(userId) {
        ...
        try{
            ...
            result.map((e) => {
                let username;
                if(userId == e.owner_id) username = userManager.getUserInfoById(e.owner_id).username
                else if(userId == e.buyer_id) username = userManager.getUserInfoById(e.buyer_id).username
                delete e.owner_id;
                delete e.buyer_id;
                e.username = username;
                return e;
