...
class chatManager {
    ...
    static sendMessage(chatId, senderId, content) { 
        ...
        try{
            ...
            let senderUsername = senderInfo.username;
            userManager.sendPushNoti(recevicerId, {
                event: 'RECV_MSG',
                data: {
                    chatId,
                    senderId,
        ...
        }catch(e){console.error(e)}finally{chatDB.close()}
    ...
    static getChatsByUserId(userId) {
        ...
        try{
            ...
            result.map((e) => {
                let username;
                if(userId == e.owner_id) username = userManager.getUserInfoById(e.buyer_id).result.username
                else if(userId == e.buyer_id) username = userManager.getUserInfoById(e.owner_id).result.username
                delete e.owner_id;
                delete e.buyer_id;
                e.username = username;
                return e;
