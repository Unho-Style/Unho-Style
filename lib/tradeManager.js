...
class tradeManager {
    ...
    static getTradesByPage(page) { 
        ...
        try {
            page = parseInt(page);
            result = tradeDB.prepare('SELECT * FROM trade WHERE status = 0 ORDER BY id DESC LIMIT ?, 20;').all((page - 1) * 19);
        }catch(e){console.error(e)}finally{tradeDB.close()}
    ...
    static getTradesByOwnerId(tradeId) { 
        ...
        let result;
        try {
            result = tradeDB.prepare('SELECT * FROM trade WHERE ownerId = ? ORDER BY timestamp DESC;').all(tradeId);
        }catch{}finally{tradeDB.close()}
        return {success: !!result, result}
    }

    static searchTrades(category, query, lastId) {
        let tradeDB = require('better-sqlite3')('./trade.db');
        lastId = lastId ?? 0;
        let result;
        try {
            if(category != -1)
                result = tradeDB.prepare('SELECT * FROM trade WHERE category = ? AND (title LIKE ? OR content LIKE ?) ORDER BY timestamp DESC LIMIT ?, 20;')
                    .all(category, `%${query}%`, `%${query}%`, lastId);
            else
                result = tradeDB.prepare('SELECT * FROM trade WHERE title LIKE ? OR content LIKE ? ORDER BY timestamp DESC LIMIT ?, 20;')
                    .all(`%${query}%`, `%${query}%`, lastId);
        }catch{}finally{tradeDB.close()}
        return {success: !!result, result}
    }
    ...
    static getTradesByBuyerId(buyerId) { 
        ...
        let result;
        try {
            result = tradeDB.prepare('SELECT * FROM trade WHERE buyer_id = ? ORDER BY timestamp DESC;;').all(buyerId);
        }catch{}finally{tradeDB.close()}
        return {success: !!result, result}
    }
