...
class userManager {
     ...
     * @param {Number} rate 평가
     */
    static setUserRate(id, rate) {
        let userDB = require('better-sqlite3')('./user.db');
        let result;
        try {
            let currRating = userDB.prepare('SELECT rate FROM users WHERE id = ?;')
            .get(id)?.rate;
            result = userDB.prepare('UPDATE users SET rate = ? WHERE id = ?;')
                .run(((currRating + rate) / 2).toFixed(1), id);
        }catch(e){
            console.log(e)
        }finally{userDB.close()}
    ...
    static subscribePushNoti(id, subInfo) {
        ...
        let msg;
        try {
            result = userDB.prepare('UPDATE users SET subInfo = ? WHERE id = ?;')
     ...
     * @param {object} data
     */
    static sendPushNoti(recevicerId, data) {
        let userDB = require('better-sqlite3')('./user.db');
        let result = false;
        try {
            let subInfo = userDB.prepare('SELECT subInfo FROM users WHERE id = ?;')
                .get(recevicerId)?.subInfo;
            if(!!subInfo) {
                console.log(JSON.parse(subInfo));
                webpush.sendNotification(JSON.parse(subInfo), JSON.stringify(data));
                result = true;
            }
        }catch(e){
            console.log(e);
    ...
    static getUserInfoByPassword(id, pw) {
        ...
        let result;
        try {
            result = userDB.prepare('SELECT * FROM users WHERE username = ? AND password = ?;').get(id, pw)
    ...
    static getUserInfoById(id) {
        ...
        let result;
        try {
            result = userDB.prepare('SELECT * FROM users WHERE id = ?;').get(id)
        }catch{}finally{userDB.close()}
        return {success: !!result, result}
    }
}
